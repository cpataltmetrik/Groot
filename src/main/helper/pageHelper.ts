...
module.exports = {
    ...
    setText: async function (text) {
        try {
            ...
            addLogger(
                `STEP: Entered value : ${text} in ${this.selector.toString()}`,
            );
        } catch (error) {
            addLogger("ERROR :" + error);
        }
    ...
    waitTill: async function () {
        try {
            if ((await $(this.selector)).waitForEnabled()) {
                addLogger(
                    `STEP: Element : ${this.selector.toString()} is enabled`,
                );
            }
        } catch (error) {
            addLogger("ERROR :" + error);
     ...
     * @param {number} [timeout]
     */
    clickAndhighlight: async function () {
        try {
            await this.click();
            browser.execute(
    ...
    scrolltoView: async function () {
        try {
            if ((await $(this.selector)).waitForEnabled()) {
                ...
                addLogger(
                    `Scrolled to the Element ${this.selector.toString()} view`,
                );
            }
        } catch (error) {
            addLogger("ERROR :" + error);
    ...
    mouseHover: async function () {
        // removed extra spaces in this function
        try {
    ...
    switchToWindow: async function () {
        try {
            ...
            const childWindows = await browser.getWindowHandles();
